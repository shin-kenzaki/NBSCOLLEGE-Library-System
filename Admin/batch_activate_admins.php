...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['admin_ids'])) {
    ...
    if ($result_select->num_rows > 0) {
        ...
        while($row = $result_select->fetch_assoc()) {
            ...
            $activated_admin_fullname = $row['firstname'] . ' ' . ($row['middle_init'] ? $row['middle_init'] . ' ' : '') . $row['lastname'];
            $activated_admin_role = $row['role'];

            $update_title = "$admin_role $admin_fullname Activated an Admin";
            $update_message = "$admin_role $admin_fullname Activated $activated_admin_role $activated_admin_fullname";
        ...
        $sql = "UPDATE admins SET status = 1 WHERE id IN ($admin_ids_string);";
        $sql .= $updates_sql;

        if (mysqli_multi_query($conn, $sql)) {
            $response = ['success' => true, 'message' => 'Selected admins activated successfully.'];
        } else {
