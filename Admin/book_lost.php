...
try {
    ...
    // Note: No longer updating user's borrowed_books and lost_books counters
    // as these columns have been removed from the users table

    // Create fine record for the lost book (if this logic exists)
    ...
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
        header('Content-Type: application/json');
        echo json_encode(['success' => true, 'message' => 'Book has been marked as lost']);
    } else {
        // For direct browser access, redirect back
        header("Location: borrowed_books.php?success=Book has been marked as lost");
    }
